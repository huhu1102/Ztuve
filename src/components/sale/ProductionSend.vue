      ...
      <el-container class="divce-mode">
        ...
        <el-main class="send-main">
          ...
          <div v-if="!showDetaillist">
                    ...
                    <div class="in-send-show">
                      ...
                      <div class="item-left">
                        ...
                        <div class="text-alight"
                             v-if="plandetail.planDetails&&plandetail.planDetails.salesPlan&&plandetail.planDetails.salesPlan.client">
                          {{plandetail.planDetails.salesPlan.client.name}}
                        </div>
                        <div class="text-alight">--:{{plandetail.planDetails.salesPlan.client.parentName}}</div>
                        <div class="text-alight" v-if="plandetail.planDetails&&plandetail.planDetails.employee">
                          {{plandetail.planDetails.employee.name}}
                        </div>
                      </div>
                      <div class="item-right">
                        <div class="right-head">
                          ...
                          <div v-for=" (item,index) in plandetail.billList.slice(-3)" :key="'m'+item.id">
                            <div class="text-left">{{index+1}}: {{item.createDate|formatDateTime}}&nbsp
                              发货量:{{item.planNumber}}
                        ...
                        <div class="right-bottom">
                          <el-tooltip content="查看发货记录">
                            <span class="fa fa-leanpub"
                                  type="primary"
                                  @click="lookSendDetail(plandetail)"
            ...
            <!--             回访记录-->
            <div v-show="isRecord">
              <div style="text-align: right;padding: 0 100px 10px 0;color: #2fc5da;cursor: pointer;">
...
<!--                        <el-tooltip content="查看回访记录">-->
<!--                            <span class="fa fa-fax"-->
<!--                                  type="primary"-->
<!--                                  @click="lookPhoneRecord(scope.row)"-->
<!--                                  style="padding:4px;margin: 4px;color: #2fc5da;"-->
<!--                                  size="mini">-->
<!--                            </span>-->
<!--                        </el-tooltip>-->
                      </div>
              ...
              >
                <template slot-scope="scope">
                  <div style="margin-top: 10px;">
                    ...
                    <span  class="fa fa-pencil"  @click="editeAsk(scope.$index,scope.row)"
                               style="padding: 3px 4px 3px 4px;margin: 2px;color: #2fc5da;"
                               size="mini">
                    </span>
                    </el-tooltip>
                    <el-tooltip content="删除">
                    <span  class="fa fa-trash"  v-if="scope.row.id==0" @click="deleteAsk(scope.$index,scope.row)"
                           style="padding: 3px 4px 3px 4px;margin-left: 5px;color: red;"
                           size="mini">
                    </span>
                    </el-tooltip>
                  </div>
    ...
    <el-dialog v-dialog-drag :title="dialogVisitTitle" style="padding: 0;" :close-on-click-modal="false"
               :visible.sync="dialogVisitVisible" @close="cancelvisit" width="900">
        ...
        <el-table
         ...
         >
          ...
          <el-table-column  label="货物状态">
            <template  slot-scope="scope" >
              <el-select v-show="scope.row.show"  @change="stateChage"  v-model="scope.row.billstatus">
                <el-option
          ...
          <el-table-column show-overflow-tooltip label="备注">
            <template slot-scope="scope">
             <el-input placeholder="请输入备注" v-show="scope.row.show"  @change="handleEdit(scope.$index, scope.row)" v-model="scope.row.note" ></el-input>
...
<!--              <el-input placeholder="请选择时间" type="date" v-show="scope.row.show" v-model="scope.row.createDate" ></el-input>-->
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <template>
                <el-tooltip content="编辑">
                  <span class="fa fa-pencil btn-left" v-if="scope.row.id!=null" @click="editRowEvent(scope.$index,scope.row)"></span>
                </el-tooltip>
                <el-tooltip >
                  <div slot="content">
                ...
                <span class="fa fa-trash btn-left" @click="cancelRowEvent(scope.row)"></span>
                </el-tooltip>
  ...
  export default {
    data() {
      return {
        ...
        manageData:'',//当前选中的生产管理
        currentPhoneRecord:'',//当前选中的发货记录；
  ...
  this.getRequest(this.getUrl)
    .then( resp => {
      ...
      if (resp && resp.status === 200 && resp.data.success) {
        ...
        _this.detailsList.map(item=>{item['show']=false;})
      ...
      visitChange(index, row) {
        ...
        this.confirmData.shortHaulMoney=row.shortHaulMoney;
        ...
        if(this.manageData!==''){
          if(this.plans&&this.plans.length){
            this.plans.forEach(function (value) {
              if(value.id===_this.manageData.id){
                console.log("plansRecord"+_this.plansRecord);
                _this.manageData=value;
                _this.plansRecord=_this.manageData.billList;
              }
            })
          }
        }
        //回访记录列表更新
        if(this.currentPhoneRecord!==""){
          this.lookPhoneRecord(this.currentPhoneRecord);
        }
      },
      preData(Data) {
        Data.forEach((plan) => {
      ...
      lookSendDetail(item) {
        ...
        this.plansRecord = item.billList;
        console.log(item.billList);
        this.isRecord = true;
      },
      backMange() {
        this.isRecord = false;
      ...
      //查看回访记录
      lookPhoneRecord(data){
        let _this = this;
        let  billId=data.id;
        this.currentPhoneRecord=data;
        this.tableLoading = true;
        this.getUrl = "/returnvisit/findByBillId/" +billId;
        this.getRequest(this.getUrl)
          .then( resp => {
              this.tableLoading = false;
              if (resp && resp.status === 200 && resp.data.success) {
                _this.detailsList = resp.data.data;
                console.log(_this.detailsList);
                if(_this.detailsList&&_this.detailsList.length){
      ...
      manageStatusFormat(row){
        console.log(row);
        var e=row.billstatus;
        return e===1?"未到":e===2?"到达":e===3?"已签收":"未知";
      },
      //编辑回访记录
      editeAsk(index,row){
       this.visit.note=row.note;
      ...
      //删除回访记录
      deleteAsk(index,row){
        this.$confirm('此操作将删除该条回访记录， 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
