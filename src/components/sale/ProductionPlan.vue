...
<template>
  <div>
    <el-container v-show="sendShow==1" style="display: flex; flex-direction: column;justify-content: flex-start;">
      ...
      <el-container class="divce-mode">
        ...
        <el-main style="padding-left: 0;padding-top: 0">
          ...
          <div>
            <div class="flex-plan" v-if="productionPlanList&&materialList">
              <div
                class="item-sales" v-if="tabsIndex==0">
                <div class="sales-add-new">
                  <el-tooltip content="添加新计划">
                                <span style=" display: block;cursor: pointer;"
                                      @click="depAddDetail(plan)">+
                                </span>
                  </el-tooltip>
                </div>
                <!--                sales-add-new-->
              ...
              <template v-if="tabsIndex==0">
                <div class="item-sales"
                     ...
                     :key="'s'+plandetail.id">
                  <div class="in-sales-show abandon">
                    <el-badge value="new" class="item">
                      <div class="compay-ifor">
                        <div style="height: 100%;width: 100%; ">
                          <div class="text-alight">
                            <div >{{plandetail.serialNumber.substring(3)}}</div>
                            <div class="plan-num-companyName" style="color: black;">
                        ...
                        <div class="plan-item-right">
                          <div style="width: 140px">
                            <div style="padding: 30px 0 0 4px;font-size: 12px;">
                              <div class="plan-num-detail">
                                ...
                                <span class="detail-text detail-right">{{plandetail.expecNum|commafy}}</span>
                              </div>
                              <div class="plan-num-detail">
                                <span class="detail-text">已下发:</span>
                                <tooltip placement="top">
                                  <div slot="content">
                                    <div v-for="(record,cindex) in plandetail.planString" :key="'send'+cindex">{{record}}</div>
                                  </div>
                                  <span class="detail-text send detail-right">{{plandetail.sendNum|commafy}}</span>
                                </tooltip>
                              </div>
                              <div class="plan-num-detail">
                                <span class="detail-text">未下发:</span>
                                <span class="detail-text detail-right">{{(plandetail.expecNum-plandetail.sendNum)<0?((plandetail.expecNum-plandetail.sendNum)|commafy)+"(超出)": (plandetail.expecNum-plandetail.sendNum)|commafy}}</span>
                              </div>
                              <div class="plan-num-detail" v-if="plandetail.shippingQuantity">
                                <span class="detail-text">已发货:</span>
                                <span class="detail-text detail-right">{{plandetail.shippingQuantity|commafy}}</span>
                              </div>
                              <div class="plan-num-detail" v-if="plandetail.resendNo">
                                <span class="detail-text">发货请求:</span>
                                <tooltip placement="top">
                                  <div slot="content">
                                    <div v-for="(walk,index) in plandetail.resendStr" :key="'w'+index">{{walk}}</div>
                                  </div>
                          ...
                          <div class="plan-note-ol">
                            <div class="note-textarea">
                              ...
                              <div class="plan-note">
                                {{plandetail.note==null?"暂无备注":plandetail.note=="null"?"暂无备注":plandetail.note}}
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <div class="item-sales"
                   v-for="(plandetail,index) in productionPlanList"
                   @mouseover="plandetail.onhover=true"
                   @mouseout="plandetail.onhover=false"
                   :key="">

                <div class="in-sales-show abandon" @click="checkState(plandetail)">
                  <div>
                    <div style="display:flex;flex-direction: row;justify-content: start;width: 100%;">
                      <div class="pro-plan-head">
                        <div class="compay-ifor " style="background-color: #A9DC90;">
                          <div class="text-alight">
                            <div style="text-align:left;"class="default-color"
                                 v-if="plandetail.planNo" @dblclick="" @click="">
                              {{plandetail.planNo}}
                            </div>

                            <div class="plan-num-companyName default-color"
                          ...
                          <div style="text-align: right;padding-right: 10px;"
                              ...
                              <span>{{plandetail.salesPlan.serialNumber.substring(3)}}
                                {{plandetail.salesPlan.editer.name}}</span>
                          </div>
                          <el-checkbox style="padding-left: 10px;" v-show="plandetail.toChange"
                                       v-model="plandetail.checked"></el-checkbox>
                        </div>
                        <div class="plan-num">
                          <div class="plan-num-left">
                            <div class="plan-item-right">
                              <div style="width: 140px;height: 100px;" class="pan-right">
                                <div style="">
                                  <div class="plan-num-detail default-color">
                                    <span class="detail-text">本次计划:</span>
                                  ...
                                  <div class="plan-num-detail">
                                    ...
                                    <el-tooltip placement="top">
                                      ...
                                      <span class="detail-text send detail-right">{{plandetail.accomplishNO|commafy}}</span>
                                    </el-tooltip>
                                  </div>
                                  <div class="plan-num-detail">
                                    <span class="detail-text">&nbsp</span>
                                  </div>
                                  <hr style="width: 86px;">
                                  <div class="plan-num-detail abandon">
                                    <span class="detail-text">未生产:</span>
                              ...
                              <div class="plan-note-ol" v-if="tabsIndex==0">
                                <div class="note-textarea">
                                  <div class="note-head">备注:</div>
                                  <div class="plan-note">
                                    {{plandetail.note==null?"暂无备注":plandetail.note=="null"?"暂无备注":plandetail.note}}
                              ...
                              <div v-else
                                   style="width: 280px;flex-direction: row;display: flex;justify-content: flex-start;">
                                <div style="width: 100px;" class="pan-right">
                                  <div style="padding: 20px 0 0 0;font-size: 12px;">
                                    <div class="plan-num-detail">
                                      <span class="detail-text">已生产:</span>
                                    ...
                                    <div class="plan-num-detail">
                                      ...
                                      <el-tooltip>
                                        <div slot="content">
                                          发货记录：
                                          <div v-if="plandetail.shippingStr&&plandetail.shippingStr.length"
                                               style="display: flex;flex-direction: column;width: auto;">
                                            <div v-for=" (item,indexAt) in  plandetail.shippingStr" :key="indexAt">
                                              {{item}}
                                            </div>
                                          </div>
                                        </div>
                                    ...
                                    <div class="plan-num-detail abandon">
                                      ...
                                      <span class="detail-text detail-right"
                                            v-if="plandetail.accomplishNO&&plandetail.sendQuantity">{{(plandetail.accomplishNO-plandetail.sendQuantity)|commafy}}</span>
                                      <span class="detail-text detail-right" v-else>0</span>
                                    </div>
                                  </div>
                                </div>
                                <div style="width: 180px;" class="pan-right">
                                  <div>
                                    <div class="plan-num-detail">
                                      <span class="detail-text">需求总数:</span>
                                      <span class="detail-text detail-right" v-if="plandetail.salesPlan">{{plandetail.salesPlan.quantity|commafy}}</span>
                                    </div>
                                    <div class="plan-num-detail">
                                      <span class="detail-text">计划总量:</span>
                                      <el-tooltip>
                                        <div slot="content">
                                          下发计划记录：
                                          <div style="display: flex;flex-direction: column;width: auto;">
                                            <div v-for=" (item,itemIndex) in  plandetail.planString" :key="itemIndex">
                                              {{item}}
                                            </div>
                                          </div>
                                        </div>
                                    ...
                                    <div class="plan-num-detail">
                                      <span class="detail-text">累计完成:</span>
                                      <el-tooltip placement="top">
                                        <div slot="content">
                                          完成记录：
                                          <div v-if="plandetail.produtRecordStr&&plandetail.produtRecordStr.length"
                                               style="display: flex;flex-direction: column;width: auto;">
                                            <div v-for=" (item,indexAt) in  plandetail.produtRecordStr" :key="indexAt">
                                              {{item}}
                                            </div>
                                          </div>
                                        </div>
                            ...
                            <div
                              ...
                              <div
                                ...
                                <div style="text-align: left;">
                                  <span v-if="tabsIndex===0&&plandetail.checkStatus!==2"
                                        class="trunk-start">新建~！！！</span>
                                  <span>{{plandetail.createDate|simpleFormatTime}}</span>
                                </div>
                                <div>
                                  <el-tooltip class="item" effect="dark"  v-if="plandetail.accomplishNO" content="生产详情" placement="top-start">
                                          <span class="fa fa-file-text"
                                                v-if="plandetail.accomplishNO"
                                                @click="productDetails(index,plandetail)"
                                                style="padding: 2px;margin: 4px;color: #2fc5da;"
                                          > </span>
                                  </el-tooltip>
                                  <el-tooltip class="item" effect="dark"   v-if="plandetail.sendQuantity" content="发货记录" placement="top">
                                      <span
                                  ...
                                  <el-tooltip class="item" effect="dark" content="发货单" placement="top">
                                          <span
                                            class="fa fa-truck"
                                            @click="sendLuggage(index,plandetail)"
                                            style="padding: 2px;margin: 4px;color: #2fc5da;"> </span>
                                  </el-tooltip>
                                  <el-tooltip class="item" effect="dark" content="发货请求" placement="top">
                                          <span
                                            class="fa fa-send"
          ...
          <div
            ...
            <div class="productio-detail">
              ...
              >
                <div
                  ...
                  <div class="company-head-item head-item-left">
                    <div>
                      <span class="a-single"
                            href="javascript:void(0)">选择客户:</span>
                    </div>
                    <div>
                      <span class="a-single"
                            href="javascript:void(0)">作业区:</span>
                    </div>
              ...
              <div style="width: 100%;padding-right: 10px; text-align: right;">
                <span> 下单人:{{salse.empName==null?'毛主管':salse.empName}}</span>
                <!--                </div>-->
              </div>
            </div>
            <div class="productio-detail">
              ...
              <div class="item-right">
                ...
                <div>
                  <el-input size="mini"
                            ...
                            :disabled="isAddPlan"
                            min="0"
                            v-model="planDetail.expCount"
                            @change="plandetailCount"
                            class="left-input"></el-input>
                </div>
              </div>
              ...
              <div class="plan-client-item">
                <div>
                  ...
                  <div>
                    <el-select
                      :disabled="isAddPlan"
                      v-model="planDetail.colorName"
                      size="mini"
                      @change="setSalesColor"
                               placeholder="请选择规格">
                      <el-option
            ...
            <div class="productio-detail">
              ...
              <div style="width: 100%;height: 200px;">
                ...
                <div v-if="serialNumbers.length">
                  <div class="item-right numbers-show" v-for="noList in serialNumbers" :key="noList.id">
                    {{noList.createDate|formatDate}}&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
                    {{noList.startNo}}-{{noList.endNo}}
                  </div>
                </div>
                <div v-else>
                  暂无
              ...
              <div class="item-right">
                ...
                <div style="width: 100%;display: inline-flex;flex-direction: row;justify-content: start;">
                  <el-input style="width: 50%;" v-model="serialNumber.startNo" size="mini"
                            @change='setbyStartNo' placeholder="起始编码..."></el-input>
                  -
                  <el-input style="width: 50%;" v-model="serialNumber.endNo" size="mini"
              ...
              <div class="plan-item-add">
                <span class="plan-item-add-btn" @click="addNumberList"> 添加 </span></div>
              <div v-for="(mynum,index) in serialEdit" :key="mynum.id" class="item-right numbers-show">
                <span class="text-overflow"> &nbsp&nbsp {{mynum.startNo}}-{{mynum.endNo}};</span>
                <span>&nbsp&nbsp
                </span>
                <span @click="editSerialNo(index)" style="cursor: pointer;padding-left: 10px"
                      class="fa fa-pencil item-span-icon"></span>
                <span @click="deletSerialNo(index)" style="padding-left: 20px; cursor: pointer;"
                      class="fa fa-trash item-span-icon"></span>
              </div>
            </div>
          </div>
    ...
    <!--销售计划-->
    <el-dialog v-dialog-drag :title="dialogMaterialTitle" style="padding: 0px;" :close-on-click-modal="false"
    ...
    <!--     产品选择界面-->
    <el-dialog v-dialog-drag :title="dialoglockTitle" style="padding: 0px;" :close-on-click-modal="false"
...
<!--      :dialog-send-visible="sendVisible"></send-things>-->
...
<script>
  ...
  export default {
    components: {ZtDialog,sendView,tagTip,manageChart,SendThings},
    data() {
      return {
        sendClients:[],
        currentManagerId:0,
    ...
    methods: {
      ...
      productDetails(index,plandetail){
        var planId=plandetail.id;
        this.currentPlandetails=plandetail;
        var that=this;

        if(planId){
          this.getRequest('/productionplandetails/findbyMangeId/'+planId)
            .then(resp=>{
              if(resp&&resp.status===200){
                console.log(resp.data);
                that.currentManagerId=resp.data;
      ...
      checkState(item) {
        ...
        if (item.checkStatus !== 2) {
          this.getRequest("/productionplandetails/checkState/" + item.id).then(resp => {
            if (resp && resp.status === 200 && resp.data.success) {
              _this.loadData();
      ...
      revokeBeg() {
        let _this = this;
        this.getRequest("/shippingrequestdetails/revokePlanBeg?id=" + this.sendDetail.id).then(resp => {
          if (resp && resp.status === 200 && resp.data.success) {
        ...
        this.$refs[addForm].validate((valid) => {
          if (valid) {
            if (_this.sendDetail.id) {
              ...
              this.postRequest("/shippingrequestdetails/updatePlan", detail).then(resp => {
                if (resp && resp.status === 200) {
              ...
              //添加
              this.tableLoading = true;
              this.postRequest("/shippingrequestdetails/addPlan?salesPlanId=" + _this.CurrentSalesId, detail).then(resp => {
      ...
      deliveryRequest(index, detail) {
        ...
        if (detail.resendId) {
          this.getRequest("/shippingrequestdetails/findById?id=" + detail.resendId).then(resp => {
            if (resp && resp.status === 200) {
      ...
      codeLenSet(e) {
        ...
        console.log(e);
        this.codeLen = e;
        if (this.serialNumber.startNo && this.serialNumber.startNo !== '') {
          this.serialNumber.startNo = that.sugestStartNo(this.serialNumber.startNo, 1)
        ...
        if (this.serialNumber.endNo && this.serialNumber.endNo !== '') {
          this.serialNumber.endNo = that.sugestStartNo(this.serialNumber.endNo, 1)
        }
      ...
      chooseMaterial() {
        console.log('选择原料了！');
        let vm = this;
        if (this.selectLockObj.id > 0) {
          const msg = this.selectLockObj.producteName;
      ...
      editeDetail(row, plan) {
        ...
        var notCount=row.salesPlan.quantity-row.salesPlan.nextQuantity;
        this.salse = {
          id: row.salesPlan.id,
          colorName: row.salesPlan.color.name,
          expCount: row.salesPlan.quantity,
          notPlanCount:notCount<0?("超额"+notCount):notCount,
        ...
        //获取站名；
        var nowShop;
        let nowShopId = that.salse.currentClientIds[that.salse.currentClientIds.length-1];
        ...
        console.log("planStatus"+row.salesPlan.planStatus);
        this.planDetail = {
          ...
          clientId: row.salesPlan.client.id,
          productId: row.salesPlan.product.id,
          id: row.id,
          createDate: row.createDate,
          productionPlanId: plan.id,
          status: row.salesPlan.planStatus,
          type: 'update',
          expectLevel: row.expectLevel,
          resendId: row.resendId == null ? 0 : row.resendId,
          resendNo: row.resendNo,
          resendStr: row.resendStr == null ? '' : row.resendStr,
      ...
      initData() {
        ...
        this.getRequest("/productionplan/basedata").then(resp => {
          if (resp && resp.status === 200) {
            ...
            _this.sendClients=_this.sendToTree(data.root.clients);
      ...
      setEndNumber: function (e) {
        console.log(e) + "**********  **********8";
        let that = this;
        let len = this.serialNumbers.length;
        //编码序列存在~~~
        if (len) {
          // 最近一次编码；
          let noObj = this.serialNumbers[0];
          this.setbyShop(noObj, e)
        } else {
          that.setCurrentEndData(this.createCoded(1, this.codeLen), e, 0);
        }
      },
      //根据最近一次编码 和 输入数量 设置编辑规则
      setbyShop(noObj, e) {
        if (noObj.startNo) {
          this.serialNumber.startNo = this.sugestStartNo(noObj.endNo, 2);
          this.serialNumber.endNo = this.sugestStartNo(noObj.endNo, parseInt(e) + 1);
        }
      },
      ...
      //根据起始编码 设置其他编码
      setbyStartNo() {
        console.log('setbyStartNo');
        this.serialNumber.startNo = this.sugestStartNo(this.serialNumber.startNo, 1)
        if (this.planDetail.actualQuantity) {
          this.serialNumber.endNo = this.sugestStartNo(this.serialNumber.startNo, this.planDetail.actualQuantity)
        }
      },
      //设置结束编码
      setbyEndNo() {
      ...
      depAddDetail() {
        ...
        let userName=this.$store._modules.root.state.user.username;
        console.log('newSalePlan'+userName);
      ...
      addNewDetail(row) {
        ...
        this.plan.id = 0;
        var notCount=row.expecNum-row.sendNum;
        this.salse = {
          id: row.id,
        ...
        var clientId=0;
         if(row.clientIds.length){
          let arr=row.clientIds.split(",")
             clientId=arr[arr.length-1];
         }
        this.planDetail = {
          'productName': row.productName,
          'specName': row.specName,
      ...
      //添加详情
      add(addDetailForm) {
        console.log("isAddPlan"+this.isAddPlan);
         if(!this.isAddPlan){
            /**新增生产计划和销售计划
             *  方法仅仅使用于i综合
             * */
               console.log("hashehe");
           this.createPlansAndSales(addDetailForm);
         }else{
           //修改方法
           this.addAndUpdate(addDetailForm)
         }



      },
      createPlansAndSales(addDetailForm){
        let _this = this;
        console.log("addbox！！！！");
        if (!(this.planDetail.actualQuantity&&this.planDetail.expCount)) {
          Message('请输入数量！');
          return false;
        }
        this.planDetail.id=0;
        this.planDetail.resendNo=0;
        this.planDetail.salesPlanId=0;
        this.planDetail.pcCode=this.planDetail.currentClientIds.join(',');
        if(this.planDetail.currentClientIds.length){
        this.planDetail.clientId=this.planDetail.currentClientIds[this.planDetail.currentClientIds.length-1];
        }
        ...
        this.$refs[addDetailForm].validate((valid) => {
          if (valid) {
            //添加
              this.tableLoading = true;
              _this.postRequest("/productionplandetails/addSalesAndplan", _this.planDetail).then(resp => {
                _this.tableLoading = false;
                if (resp && resp.status === 200) {
     ...
     addAndUpdate(addDetailForm){
       ...
       this.$refs[addDetailForm].validate((valid) => {
         if (valid) {
           if (this.planDetail.id) {
             //更新
             _this.tableLoading = true;
             _this.postRequest("/productionplandetails/update", _this.planDetail).then(resp => {
               _this.tableLoading = false;
               if (resp && resp.status === 200) {
                 _this.dialogFormVisible = false;
                 _this.loadData();
             ...
             //添加
             this.tableLoading = true;
             _this.postRequest("/productionplandetails/add", _this.planDetail).then(resp => {
               _this.tableLoading = false;
               if (resp && resp.status === 200) {
                 _this.dialogFormVisible = false;
                 _this.loadData();
          ...
          .then(
            resp => {
              ...
              if (resp && resp.status === 200 && resp.data.success) {
                ...
                _this.productionPlanList = _this.preForData(list);//
                //没有当天的
      ...
      loadMaterial() {
        let _this = this;
        this.tableLoading = true;
        this.getRequest("/sales/findfornoplan?page="
      ...
      preForData(data) {
        ...
        if (data && data.length) {
          for (let i = 0, n = data.length; i < n; i++) {
            let e = data[i];
            //计划记录
            if(e.planString&&salesPlan.planString){
              e.planString = e.salesPlan.planString.split(";")
            }else{
              e.planString=[]
            }
            //发货记录
            if (e.salesPlan && e.salesPlan.shippingStr) {
              e.shippingStr = e.salesPlan.shippingStr.split(";")
            } else {
              e.shippingStr = [];
            }
            if(e.salesPlan&&e.salesPlan.produtRecordStr){
              e.produtRecordStr=e.salesPlan.produtRecordStr.split(";")
              }
      ...
      choosCompay(item, index) {
        ...
        this.getRequest("/productionplan/getPlanDetails?clientId=" + companyId).then(resp => {
          ...
          console.log(resp);
          if (resp && resp.status === 200) {
            _this.serialNumbers = resp.data.root.clientList;
      ...
      //当选择产品确认后的设置
      setlockData() {
        console.log('setlockData');
        let that = this;
        let v = that.selectLockObj;
        console.log(v);
        that.planDetail.productName = v.producteName;
        this.planDetail.productId=v.id;
        that.planDetail.proId = v.id; //选取的物料Id
        that.planDetail.specification = '';
        that.specslist = v.specs || [];
        if (that.specslist.length) {

          that.planDetail.specId = that.specslist[0].id;
          that.planDetail.specName=that.specslist[0].id;
          that.salse.specName=that.specslist[0].name;
        }

        that.colorlist = v.colors || [];
        if (that.colorlist.length) {
          that.planDetail.colorId = that.colorlist[0].id;
          that.planDetail.colorName=that.colorlist[0].id;
          that.salse.colorName=that.colorlist[0].name;
        }
        that.planDetail.productNo = v.proType;
        //设置销售计划中产品信息 产品名
        this.salse.productName=v.producteName;
      ...
      setSalesSpec(e){
        let that=this;
        console.log(e);
        this.filteName(this.specslist,e,'spec')

      },

      setSalesColor(e){
        console.log(e);
        this.filteName(this.colorlist,e,'color')
      },
      filteName(arr,id, type){
        let that=this;
        if(arr&&arr.length){
          for (let i = 0,n=arr.length; i <n ; i++) {
            if(id===arr[i].id){
              if(type==='spec'){
              that.salse.specName=arr[i].name;
              }else if(type==="color"){
               that.salse.colorName=arr[i].name
              }
            }
          }
        }
