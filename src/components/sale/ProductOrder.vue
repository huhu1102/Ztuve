...
<!--            <el-button type="primary" size="mini" icon="el-icon-plus" @click="newSalePlan">创建销售计划</el-button>-->
        ...
        <el-main class="send-main">
          <transition name="slide-fade">
            <div class="send-main-div"
              ...
              <el-row>
                ...
                <el-col :span='8'>
                  客户:<el-input prefix-icon="el-icon-edit" v-model="keyClientName" size="mini" class="send-input"
                               placeholder="客户名。。。">
                </el-input>
                </el-col>
                <el-col :span='8'>
          ...
          <div>
            <el-table :cell-style="{padding:'2px',fontSize:'12px'}" :data="ordersList" border fit style="width: 100%;font-size: 12px;">
              ...
              <el-table-column align="left" fixed label="合同编号">
                <template slot-scope="scope">
              ...
              <el-table-column align="left" fixed label="创建时间">
                <template slot-scope="scope">
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-tooltip content="编辑">
                    <span  class="fa fa-pencil" @click="editOder(scope.$index,scope.row)" style="padding: 3px 4px 3px 4px;margin: 2px;color: #2fc5da;">
                    </span>
                  </el-tooltip>
                   <el-tooltip content="删除">
                     <span class="fa fa-trash" style="padding: 3px 4px 3px 4px;margin: 2px;color: red;"
                                @click="delOder(scope.row)">
                     </span>
                   </el-tooltip>
                </template>
              </el-table-column>
   ...
   <!--产品-->
    ...
    <el-dialog v-dialogDrag :title="dialogOrdersTitle" style="padding: 0;" :close-on-click-modal="false"
               :visible.sync="dialogordersVisible" @close="clearOderAddloadDate" width="960px">
                ...
                <el-col :span='8'>
                  <el-form-item label="订单编号:" prop="orderNo">
                    <el-input prefix-icon="el-icon-edit" v-model="order.orderNo" size="mini" style="width: 200px"
                             disabled placeholder="订单编号"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            </div>
              ...
              <el-row v-if="order.hasContract">
                ...
                <el-col :span='8'>
                  <el-form-item label="合同名称:" prop="contractName">
                    <el-input prefix-icon="el-icon-edit" v-model="order.contractName" size="mini" style="width: 200px"
          ...
          <div>
            <el-row>
              ...
              <el-col style="text-align: start;">
                <el-button type="primary" style="padding: 6px 4px;margin: 5px;" @click="chooseSales">选择生产计划</el-button>
                <div v-if="selectSales.salesPlan&&selectSales.salesPlan.client">
                <span style="margin: 10px;display: block;">
                {{selectSales.salesPlan.client.name}}
                 {{selectSales.salesPlan.product.producteName}}
                  |{{selectSales.salesPlan.specification.name}}
                 |{{selectSales.salesPlan.color.name}}
                 需求数量:
                 {{selectSales.salesPlan.quantity}};
                       下发人:
                {{selectSales.salesPlan.editer.name}}
                </span>
                </div>
            ...
            <el-table :data="items" :cell-style="{padding:'2px',fontSize:'12px'}" fit border style="width: 100%">
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-tooltip content="编辑" >
                    <span class="fa fa-pencil" @click="editeItem(scope.$index,scope.row)" style="padding: 3px 4px 3px 4px;margin: 2px;color: #2fc5da"
                               >
                    </span>
                  </el-tooltip>
                 <el-tooltip class="删除">
                   <span class="fa fa-trash"  style="padding: 3px 4px 3px 4px;margin: 2px;color: #2fc5da"
    ...
    <el-dialog v-dialogDrag :title="dialogUnsalesTitle" style="padding: 0;" :close-on-click-modal="false"
      ...
      <div style="width: 100% " v-if="dialogUnSaleVisible">
        ...
        <el-table :data="orderitems":cell-style="{padding:'2px',fontSize:'12px'}" fit border style="width: 100%" @row-click="showitemRow">
          ...
          <el-table-column align="left" fixed label="编号">
            <template slot-scope="scope">
              {{scope.row.planNo}}
            </template>
          </el-table-column>
          ...
          <el-table-column align="left" fixed label="规格">
            <template slot-scope="scope">
              {{scope.row.salesPlan.specification.name}}
            </template>
          </el-table-column>
          ...
          <el-table-column align="left" fixed label="需求数量/生产完成数量">
            <template slot-scope="scope">
              {{scope.row.actualQuantity}}
              /{{scope.row.accomplishNO}}
            </template>
          </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //选择销售计划确认
      chooseUnSale() {
        this.orderItem.productNo =this.selectSales.salesPlan.quantity
      ...
      addSalesItem() {
        ...
        if (this.items && this.items.length) {
          for (let i = this.items.length - 1; i >= 0; i--) {
            let me = that.items[i];
            if (me.productionPlanDetailsIds === this.selectSales.id) {
              me.productNo = that.orderItem.productNo;
              me.totalMoney = that.orderItem.totalMoney;
              me.unitPrice = that.orderItem.unitPrice;
              if (me.tap === 'ood') {
                me.tap = 'update';
              }
              flag = true;
        ...
        if (!flag) {
          this.orderItem.productionPlanDetailsIds = this.selectSales.id;
          this.orderItem.tap = 'new';
          this.maxId++;
          this.orderItem.resourcesNumber = this.maxId;
          this.orderItem.sid = this.maxId;
          this.items.push(this.orderItem);
        }
        this.order.totalMoney=0;

        this.items.map((item)=>{that.order.totalMoney+=item.totalMoney; })
        this.clearItem();


      },
      clearOderAddloadDate(){
         this.clearItem();
         this.clearOder();
        this.items=[];
        this.loadmyData()
      ...
      editeItem(index, row) {
        console.log("row" + row);
        this.orderItem = row;
        this.selectSales.id = row.productionPlanDetailsIds
      },
      //修改订单
      editOder(index, row) {
        let that = this;
        ...
        this.order.contractName=row.contractName;
        this.order.contractNumber=row.contractNumber;
        this.order.orderNo=row.orderNo;
        this.order.hasContract=row.hasContract?1:0;
        this.order.empId=row.empId;
        this.order.qualityDeposit=row.qualityDeposit;
        this.order.tax=row.tax;
        this.order.totalMoney=row.totalMoney;
        this.order.signContractDate=row.signContractDate;
        this.order.state=row.orderStatus;
        this.order.notes=row.notes;
        this.order.clientArr = row.clientArr;
        this.order.cliId = row.clientArr.split(',').map(item=> {return  parseInt(item);} );
         //订单详情
        let itemsArr=[];
        this.dialogOrdersTitle = '修改订单';
        if (row.salesOrderDetails && row.salesOrderDetails.length) {
          itemsArr=row.salesOrderDetails.map((item,index,arr)=>{
               let  itemDetail={
                 tap:"ood",
                 sid:index+1,
                 ...
                 productionPlanDetailsIds:item.productionPlanDetails.id,
               };
               return itemDetail;

          });
          that.maxId = row.salesOrderDetails.length;
        }
        this.items = itemsArr;
      ...
      addOrder(addOrderForm) {
        ...
        plans.clientArr = plans.cliId.join(',');
        plans.cliId = plans.cliId.pop();
        let currentDetail = this.items.concat(this.deItem);
        plans.orderDetails = this.jsonToString(currentDetail);
          ...
          if (valid) {
            if (plans.id) {
              ...
              this.postRequest("/orders/update", plans).then(resp => {
                ...
                let re=resp || resp.status === 200 || resp.data.success;
                if (!re) {
                  _this.errorInfo();
                }
      ...
      doDelete(ids){
        this.tableLoading = true;
        let _this = this;
        this.getRequest("/orders/delete?id=" + ids).then(resp=> {
          _this.tableLoading = false;
      ...
      currentSelectShop(e) {
        console.log(e + '客户');
        this.salse.clientId = Array.from(e);
      },
      currentSelectCliets(e) {
        this.order.cliId = Array.from(e);
      },
      //**.....
      //新建销售计划
      chooseMaterial() {
        console.log('选择了！6666');
        let vm = this;
      ...
      loadMaterial() {
        ...
        this.getRequest("/product/findbypage?page=" + this.currentMaterialPage + "&size=10&queryName=" + this
          ...
          if (resp && resp.status === 200&&resp.data.success) {
            _this.materialList = resp.data.data;
        ...
        this.$refs[addSaleForm].validate((valid) => {
          if (valid) {
            if (plans.id) {
              ...
              //添加
              this.tableLoading = true;
              if (plans.clientId === 0 || plans.clientId === '') {
                plans.clientId = 0;
              }
              console.log(plans);
      ...
      loadmyData() {
        let _this = this;
        this.tableLoading = true;
        this.getRequest( "/orders/findbypage?page="+ this.currentPage
      ...
      loadSalse() {
        ...
        let clientid='';
        this.tableLoading = true;
        if (this.order.cliId && this.order.cliId.length) {
          clientid = this.order.cliId[this.order.cliId.length-1];
        }
        if (clientid==='') {
          Message("请选择客户@--！");
          return false;
        } else {
          this.getRequest("/productionplandetails/findForOrder?page="
            + this.currentItemPage
            + "&size=10&clientId=" + clientid
          ).then(resp => {
            this.tableLoading = false;
            if (resp && resp.status === 200 && resp.data.success) {
              _this.orderitems = resp.data.data;
              _this.totalItemCount = resp.data.total;
              _this.dialogUnSaleVisible = true;
            }else{
              Message('查询错误！')
            }
          })
