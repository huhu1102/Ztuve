<template>
  <div>
      ...
      <el-header>
        ...
        <div style="text-align: left;margin-left: 20px;">
          ...
          <span class="fa fa-list-ul" :class="{stamp:tap===1}" @click="tap=1"
                style="margin-left: 30px;cursor: pointer;"> 列表展示</span>
          <span style="cursor: pointer;font-weight: 400;color: blueviolet;margin-left: 20px;" @click="andCode">添加客户流派</span>
          <tooltip content="刷新">

          <span style="color: #85ce61; cursor: pointer;;margin-left: 20px;" class="fa fa-refresh" @click="initData"></span>
          </tooltip>
    ...
    <el-container v-show="!mainShop">
      <div style="width: 900px;display: flex;flex-direction: column;flex-wrap: nowrap;justify-content: center;align-items: center;">
        <div style="height: 100%;width: 100%;padding:20px 0 0 180px">
          <div style="padding: 12px;">
            <span>客户信息维护</span>
            <el-tooltip class="item" effect="dark" content="取消" placement="right-start">
            <span  style="margin-left: 60px;color: red;cursor:pointer;" class="fa fa-rotate-left" @click="cancelOpt"></span>
          </el-tooltip>
          </div>
        <el-form :model="company" :rules="rules" ref="addDataForm" size="mini" label-width="90px">
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="流派:" prop="types">
                <el-select v-model="company.types" filterable placeholder="----请选择流派----"
                           class="send-input">
                  <el-option
          ...
          <el-row
            ...
            <el-dialog   v-dialogDrag width="50%" style="margin-top: 20px;" :visible.sync="dialogImageVisible">
          ...
          <div  class="dialog-footer">
            <el-button @click="cancelOpt"   @keyup.Esc.native="cancelOpt">取 消</el-button>
...
//上传图片
      handleRemove(file) {
        ...
        if (this.imageUrlList.length) {
          this.imageUrlList.forEach(function (item, i) {
            if (item.id === file.id) {
              that.imageUrlList.splice(i, 1)
            }
          })
        }
      },
      ...
      getBaseData() {
        ...
        _this.getRequest("/client/baseData").then(resp => {
          if (resp && resp.status === 200 && resp.data.success) {
            ...
            _this.typeslist = types;
              this.typeslist= data.root.typeslist;
          }
        })
      ...
      showEditEmpView(row) {
        ...
        this.company.types=parseInt(row.types);
        this.mainShop = false;
      },
      addData(addDataForm) {
        let _this = this;
        this.$refs[addDataForm].validate((valid) => {
          if (valid) {
            ...
            delete cops.parent;
            console.log(cops);
            cops.uploadFiles = _this.jsonToString(_this.imageUrlList);
            if (_this.company.id) {
              //更新
