              ...
              <div style="display: flex;justify-content: flex-start;align-items: center;">
                <div style="display: flex;justify-content: center;align-items: center;width: 25px;height: 25px">
                  <div
                    ...
                    v-if="msg.msgState==1"></div>
                </div>
                <span style="width: 600px;text-align: left">{{msg.msgContent.title}}</span>
                <span v-if="msg.msgState==3"style="text-align: left">已撤销</span>
                <el-tag>{{msg.msgContent.createDate|formatDateTime}}</el-tag>
              </div>
            </template>
            <div
  ...
  export default{
    ...
    methods: {
      handleChange(newVal){
        ...
        var _this = this;
        this.postRequest("/chat/markread", {flag: this.mid}).then(resp=> {
      ...
      allRead(){
        ...
        this.postRequest("/chat/markread", {flag: -1,}).then(resp=> {
          if (resp && resp.status == 200) {
